# /////////////////////////////////////// FONCTION PASSAGE PIETON NORMAL ////////////////////////////////////////
import numpy as np 
import os
import matplotlib.pyplot as plt
from scipy.stats import norm  # pour une distribution normale
from Parap import nb_pop,answr,objectifx,objectify,N,Nmax,vx0,vy0,lmax,lmin,eps,alpha,mu,ramp,v0,r0,rf,Nt,dt,L,x,y,vx,vy,fx,fy,xb,yb,vxb,vyb,I
    ...
    for z in range(nb_pop):
        ...
        for k in range(0,N[z]):                    
            ...
            if (x[k][z]<= rb and y[k][z]<=rb):          # Coin bas gauche 
                nb[z] += 1   
                xb[nb[z]][z],yb[nb[z]][z] = x[k][z]+1.,y[k][z]+1.
                vxb[nb[z]][z],vyb[nb[z]][z] = vx[k][z],vy[k][z]
                
            if (x[k][z]>= 1.-rb and y[k][z]<=rb):       # Coin bas droit 
                nb[z] += 1   
                xb[nb[z]][z],yb[nb[z]][z] = x[k][z]-1.,y[k][z]+1.
                vxb[nb[z]][z],vyb[nb[z]][z] = vx[k][z],vy[k][z]
                
            if (x[k][z]<=rb and y[k][z]>=1.-rb):        # Coin haut gauche
                nb[z] += 1   
                xb[nb[z]][z],yb[nb[z]][z] = x[k][z]+1.,y[k][z]-1
                vxb[nb[z]][z],vyb[nb[z]][z] = vx[k][z],vy[k][z]
                
            if (x[k][z]>= 1.-rb and y[k][z]>=1.-rb):    # Coin haut droit 
                nb[z] += 1   
                xb[nb[z]][z],yb[nb[z]][z] = x[k][z]-1.,y[k][z]-1.
                vxb[nb[z]][z],vyb[nb[z]][z] = vx[k][z],vy[k][z]    
        ...
        for i in range(N[j]):
            ...
            if (x[i][j] >= L or x[i][j] <= 0.) :
                if j == 0:
                    ...
                    vy[i][j] = vy0[j]
                if j == 1:
                    x[i][j] = np.random.uniform(lmin, lmax)
                    y[i][j] = L
                    vx[i][j] = vx0[j]
                    vy[i][j] = vy0[j]
